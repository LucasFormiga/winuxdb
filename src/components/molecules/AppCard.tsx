import { Image as ImageIcon, Users, Calendar, Hash, ArrowRight, Sparkles } from 'lucide-react'
    ...
    <Card className="group flex h-full flex-col overflow-hidden border-border/60 bg-card/80 transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_24px_60px_rgba(0,0,0,0.5)]">
          ...
          <div className="absolute top-3 right-3 flex flex-col items-end gap-2 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
             <div className="flex items-center gap-1 rounded-full bg-black/60 px-2 py-1 text-[0.6rem] font-bold text-white backdrop-blur-md border border-white/10">
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className={`size-1 rounded-full ${i < app.popularity ? 'bg-yellow-400' : 'bg-white/20'}`} />
                ))}
             </div>
          </div>
        </div>
      ...
      <CardContent className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 border-y border-border/40 py-3">
          <div className="space-y-1">
            <p className="text-[0.6rem] uppercase tracking-wider text-muted-foreground/70 font-semibold">{t('version')}</p>
          ...
          <div className="space-y-1">
            <p className="text-[0.6rem] uppercase tracking-wider text-muted-foreground/70 font-semibold">{t('release')}</p>
            <div className="flex items-center gap-1 text-xs font-mono">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="rounded-md border-border/40 bg-muted/30 px-1.5 py-0 font-mono text-[0.6rem] uppercase tracking-wider text-muted-foreground">
            {tCat(app.category)}
          </Badge>
          <Badge variant="outline" className="rounded-md border-border/40 bg-muted/30 px-1.5 py-0 font-mono text-[0.6rem] uppercase tracking-wider text-muted-foreground">
            {tLic(app.license)}
            ...
            <div className="rounded-xl border border-primary/10 bg-primary/5 p-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {app.recommendedAlternatives.slice(0, 2).map((alt) => (
                  <span key={alt} className="text-[0.7rem] text-foreground/80 hover:text-primary transition-colors cursor-default">
                    {alt}
                  </span>
                ))}
